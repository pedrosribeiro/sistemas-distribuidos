...
KEY_PATH = "app/ms_reserva/keys/public_key.pem"
...
def processar_pagamento(ch, method, properties, body):
    ...
    if verify_signature(mensagem_bytes, assinatura, public_key_pagamento):
        logging.info(f"[MS RESERVA] Assinatura VERIFICADA de '{fila}'")
        status = resultado["status"]
        atualizar_reserva_status_pagamento(resultado["reserva_id"], status)
    else:
        logging.info(f"[MS RESERVA] Assinatura INVÁLIDA recebida na fila '{fila}'")
